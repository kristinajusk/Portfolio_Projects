...
SELECT s.store_id
	, COUNT(p.rental_id) AS 'Number Of Sales'
...
SELECT t1.store_id
	, CONCAT('$', ' ', `Total Profit`) AS 'Total Profit'
    , t1.`Unique Customers`
    ,  CONCAT('$',' ', ROUND(`Total Profit`/`Unique Customers`,2)) AS 'Average Profit Per Customer'
    , t1.`Number Of Days In Business`
    ,  CONCAT('$',' ', ROUND(`Total Profit`/`Number Of Days In Business`,2)) AS 'Average Profit Per Day'
    ,  COUNT(DISTINCT i.film_id) AS 'Number Of Unique Movies'
FROM temp_Task1 AS t1
JOIN inventory AS i ON i.store_id = t1.store_id
GROUP BY t1.store_id, t1.`Total Profit`, t1.`Unique Customers`, t1.`Number Of Days In Business`
...
SELECT f.film_id
	, CONCAT(f.title, ' ', ' ( ', cat.name, ' ) ') AS 'Movie'
    , SUM(p.amount) AS 'Total Profit'
    , CASE
    WHEN cat.name = 'Horror'
		THEN 'Horror movies always has lower rent.'
	WHEN cat.name = 'Sport' AND f.rating = 'R'
...
WHERE cat.name NOT LIKE 'Horror'
AND cat.name NOT LIKE 'Sports'
...
SELECT CONCAT('( ', f.film_id, ' ) ' , ' ', f.title) AS 'Movie'
    ...
    , SUM(p.amount) AS 'Total Profit'
FROM film AS f
LEFT JOIN temp_Task3 AS t3 ON t3.film_id = f.film_id
JOIN film_category AS fc ON fc.film_id = f.film_id
JOIN category AS cat ON cat.category_id = fc.category_id
JOIN inventory AS i ON i.film_id = f.film_id
JOIN rental AS r ON r.inventory_id = i.inventory_id
JOIN payment AS p ON p.rental_id = r.rental_id
WHERE t3.`Possible Reason` NOT LIKE 'Unknown'
GROUP BY f.film_id
	, cat.name
